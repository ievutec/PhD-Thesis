...
%DIF ADD Corrections/main.tex   Fri Mar 29 15:33:59 2024
%
...
Adiabatic protocols are employed across a variety of quantum technologies, from implementing state preparation and individual operations that are building blocks of larger devices, to higher-level protocols in quantum annealing and adiabatic quantum computation. The main drawback of adiabatic processes, however, is that they require prohibitively long timescales\DIFdelbegin \DIFdel{, leading }\DIFdelend \DIFaddbegin \DIFadd{. This generally leads }\DIFaddend to losses due to decoherence and heating \DIFaddbegin \DIFadd{processes}\DIFaddend . The problem of speeding up \DIFdelbegin \DIFdel{these processes }\DIFdelend \DIFaddbegin \DIFadd{system dynamics }\DIFaddend while retaining the adiabatic condition has garnered a large amount of interest, resulting in a whole host of diverse methods and approaches \DIFdelbegin \DIFdel{that aim to do just that}\DIFdelend \DIFaddbegin \DIFadd{made for this purpose}\DIFaddend . Most of these \DIFaddbegin \DIFadd{methodologies }\DIFaddend are encompassed by the fields of quantum optimal control and shortcuts to adiabaticity (\acrref{STA}), which are in themselves complementary approaches. Optimal control \DIFaddbegin \DIFadd{often }\DIFaddend concerns itself with \DIFaddbegin \DIFadd{the design of }\DIFaddend control fields for steering system dynamics \DIFdelbegin \DIFdel{in the minimum allowed }\DIFdelend \DIFaddbegin \DIFadd{while minimising the use of some resource, like }\DIFaddend time, while the goal of \acrref{STA} is to retain the adiabatic condition upon speed-up.
...
A particularly useful type of controlled process that we would like to be able to perform is an \emph{adiabatic} process, which involves slowly changing some parameter affecting a quantum system, \@e.g.~the strength or direction of an electromagnetic field. The `slow' part here is required to keep the system from getting excited out of the `instantaneous' energy level that it starts in. Think of \DIFdelbegin \DIFdel{slowly rotating }\DIFdelend a magnetic field \DIFaddbegin \DIFadd{slowly rotating through some angle }\DIFaddend such that a \DIFdelbegin \DIFdel{magnet }\DIFdelend \DIFaddbegin \DIFadd{bar magnet placed in said field }\DIFaddend always stays aligned with \DIFdelbegin \DIFdel{the field}\DIFdelend \DIFaddbegin \DIFadd{it}\DIFaddend . If the \DIFdelbegin \DIFdel{change }\DIFdelend \DIFaddbegin \DIFadd{rotation }\DIFaddend happens too quickly, the magnet overshoots in the direction of the changing field. An analogous process happens in the quantum case, where the quantum state `jumps' out of its energy level. For many applications of quantum technologies, we would like to avoid such jumps, hence we perform adiabatic (slow) transformations.
...
The thesis is divided into four parts, prefaced by this introduction. \DIFdelbegin \DIFdel{Part \ref{part:background} }\DIFdelend \DIFaddbegin \textbf{\DIFadd{Part \ref{part:background}}} \DIFaddend introduces key background concepts relevant to the new results discussed later in the thesis: quantum adiabaticity and quantum optimal control or \acrref{QOCT}. \DIFdelbegin \DIFdel{In Ch.~\ref{chap:2_adiabaticity}}\DIFdelend \DIFaddbegin \DIFadd{First}\DIFaddend , we discuss the concept of an adiabatic quantum process, with particular focus in Sec.~\ref{sec:2.1.2_adiabatic_condition} on what it means for a change in the Hamiltonian parameters to be `slow enough' to be adiabatic. We cover how non-adiabatic effects are generated by an operator known as the adiabatic gauge potential \DIFaddbegin \DIFadd{(}\DIFaddend or \acrref{AGP}\DIFdelbegin \DIFdel{in Sec.~\ref{sec:2.2_AGP} and then in Sec.~\ref{sec:2.3_CD} we }\DIFdelend \DIFaddbegin \DIFadd{) and subsequently }\DIFaddend introduce the concept of a counterdiabatic drive. We then discuss the difficulties of obtaining an exact counterdiabatic drive for a given Hamiltonian and introduce several existing approximations of \acrref{CD}\DIFdelbegin \DIFdel{in Sec. ~\ref{sec:2.4_approximate_CD}, including }\DIFdelend \DIFaddbegin \DIFadd{. This includes }\DIFaddend \acrref{LCD}, which \DIFdelbegin \DIFdel{continues to play }\DIFdelend \DIFaddbegin \DIFadd{plays }\DIFaddend a large part in the rest of the thesis. \DIFdelbegin \DIFdel{Then, in Ch.~\ref{chap:3_Quantum_Optimal_control}, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend introduce \acrref{QOCT}, beginning with the mathematical foundations of optimal control as well as several popular numerical optimisation methods\DIFdelbegin \DIFdel{in Sec. ~\ref{sec:3.1_structure_quantum_control}. We then }\DIFdelend \DIFaddbegin \DIFadd{. We }\DIFaddend discuss how optimal control techniques can be applied specifically to quantum systems \DIFdelbegin \DIFdel{in Sec.~\ref{sec:3.2_Quantum_optimal_control} and finish the chapter by describing }\DIFdelend \DIFaddbegin \DIFadd{and describe }\DIFaddend several \acrref{QOCT} methods that are implemented in order to acquire the results presented later in the thesis.
...
In \DIFdelbegin \DIFdel{Part~\ref{part:COLD}}\DIFdelend \DIFaddbegin \textbf{\DIFadd{Part~\ref{part:COLD}}}\DIFaddend , we introduce the main new material of the thesis: the \acrref{COLD} method \DIFdelbegin \DIFdel{in Ch.~\ref{chap:4_COLD} }\DIFdelend and its extension using \DIFdelbegin \DIFdel{different, }\DIFdelend \DIFaddbegin \DIFadd{several }\DIFaddend \acrref{AGP}-inspired cost functions\DIFdelbegin \DIFdel{in Ch. ~\ref{chap:5_cd_as_costfunc}. In Ch.~\ref{chap:4_COLD} }\DIFdelend \DIFaddbegin \DIFadd{. First, }\DIFaddend we discuss the ways in which \acrref{LCD} and quantum optimal control methods can be combined to obtain better results than either approach alone, and how that follows from the dependence of the counterdiabatic drive on the path of the Hamiltonian in parameter space. We expand on the optimal control methods used for \acrref{COLD} \DIFdelbegin \DIFdel{in Sec.~\ref{sec:4.2_COLD_QOCT}. In Ch.~\ref{chap:5_cd_as_costfunc} we then }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend introduce the idea of using information about the counterdiabatic drive itself, like its \DIFdelbegin \DIFdel{maximal amplitude value}\DIFdelend \DIFaddbegin \DIFadd{total power across the driving time}\DIFaddend , as a metric for optimising the control pulse in \acrref{COLD} and for the case where no \acrref{LCD} is applied. 

\DIFdelbegin \DIFdel{After a detailed introduction of the methods, we then implement them }\DIFdelend \DIFaddbegin \DIFadd{In }\textbf{\DIFadd{Part.~\ref{part:applications}}} \DIFadd{we demonstrate implementations of the new methods }\DIFaddend in numerical simulations of several example quantum systems\DIFdelbegin \DIFdel{in Part. ~\ref{part:applications}. In }\DIFdelend \DIFaddbegin \DIFadd{. First, in }\DIFaddend Ch.~\ref{chap:6_Applications_fidelity} we present and discuss results obtained when applying \acrref{COLD} to a simple two-spin annealing protocol\DIFdelbegin \DIFdel{in Sec.~\ref{sec:5.1_2spin_annealing}}\DIFdelend , the Ising spin chain of varying lengths\DIFdelbegin \DIFdel{in Sec.~\ref{sec:5.2_Ising_chain}}\DIFdelend , the case of population transfer in a synthetic lattice\DIFdelbegin \DIFdel{in Sec.~\ref{sec:5.3_synthetic} }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend and finally for the preparation of maximally entangled GHZ states in the setting of frustrated spin systems\DIFdelbegin \DIFdel{in Sec. ~\ref{sec:6.4_ghz_states}. }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend We compare the results obtained with \acrref{COLD} to those obtained using un-optimised \acrref{LCD} as well as different optimal control pulses with no counterdiabatic component. In Ch.~\ref{chap:7_higher_order_agp} we do the same but implement \acrref{CD}-inspired cost functions in the optimisation of \acrref{COLD} and plain optimal control instead of using fidelity or (as in the case of GHZ state preparation) entanglement as optimisation metrics. We present results for the two-spin annealing case\DIFdelbegin \DIFdel{in Sec.~\ref{sec:7.1_two_spin_ho}}\DIFdelend , the Ising spin chain\DIFdelbegin \DIFdel{in Sec.~\ref{sec:7.2_ising_ho_lcd} }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend and finally for the GHZ state preparation protocol in a system of frustrated spins\DIFdelbegin \DIFdel{in Sec. ~\ref{sec:7.3_ghz_ho}. }\DIFdelend \DIFaddbegin \DIFadd{, to compare and contrast to the case where optimisation is based on final state fidelity. }\DIFaddend We discuss when such optimisation metrics may be better than those used in Ch.~\ref{chap:6_Applications_fidelity} and in which cases they might fail.

Finally, in \DIFdelbegin \DIFdel{Part~\ref{part:conclusion} }\DIFdelend \DIFaddbegin \textbf{\DIFadd{Part~\ref{part:conclusion}}} \DIFaddend we conclude with a summary of the thesis \DIFdelbegin \DIFdel{in Ch.~\ref{chap:8_Summary} }\DIFdelend and an outlook into future research directions that are left to be explored\DIFdelbegin \DIFdel{in Ch. ~\ref{chap:9_Future_directions}. }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend A diagram of the thesis structure can be found in Fig.~\ref{fig:thesis_overview}, linking the relevant parts together.
...
\DIFdelend \DIFaddbegin \DIFadd{dynamic (or dynamical) phase}\DIFaddend . 
    ...
    So to understand adiabaticity, we need to understand what conditions lead to the case where \DIFaddbegin \DIFadd{the additional term in Eq.~\eqref{eq:coefficient_exact} is small enough to be neglected, or:
    }\DIFaddend \begin{equation}\label{eq:adiabaticity_condition}
    ...
    In the above, we can see that when the integral on the RHS is 0, we recover the result in Eq.~\eqref{eq:c_adiabatic}. The intuition is that when the integral is sufficiently small, the adiabatic condition is valid and the system will follow the instantaneous eigenstate. Since the integral is made up of a sum of terms of the same form, we can focus on determining the bound on one of them. \DIFdelbegin \DIFdel{Representing }\DIFdelend \DIFaddbegin \DIFadd{We can represent }\DIFaddend the integral as:
    ...
    \end{equation}
    where we used the result from Eq.~\eqref{eq:hamiltonian_derivative}. \DIFdelbegin \DIFdel{The integral doesn't can be simplified a lot }\DIFdelend \DIFaddbegin \DIFadd{It may be simplified significantly }\DIFaddend by using the fact that:
    \begin{equation}
...
\DIFdelend To see this, take any quantum state written in some basis, \@e.g.~$\ket{\psi} = \sum_n \psi_n \ket{n}$. Then in the moving frame basis we have:
    ...
    What we have seen so far is that when we try to solve the Schr\"{o}dinger equation for a quantum system evolving under a time-dependent Hamiltonian in the basis of the moving frame, \@i.e.~in the basis where the time-dependent Hamiltonian is diagonalised, we find that the evolution happens under a `decorated' Hamiltonian composed of the diagonalised $\Tilde{H}$ and an additional operator generally known as the adiabatic gauge potential\DIFdelbegin \DIFdel{or }%DIFDELCMD < \acrref{AGP}%%%
\DIFdelend . We found that in the lab frame, it is the derivative operator with respect to the time-dependent parameters driving the Hamiltonian. What remains is to link this to our discussion of adiabaticity and the adiabatic condition of Section \ref{sec:2.1_adiabatic_theorem}.
...
%DIFDELCMD <             i\hbar \dotlambda \partial_{\lambda} R(\lambda) &= H_{\rm t-less} (\lambda) R(\lambda), \\
%DIFDELCMD <             \Rightarrow H_{\rm t-less} (\lambda) &= i\hbar \dotlambda (\partial_{\lambda} R(\lambda)) \adj{R}(\lambda).
...
\DIFdelbegin \DIFdel{which is reflected in the fact that $H_1$ is nothing more than }\DIFdelend \DIFaddbegin \DIFadd{As previously, recall that the additional }\acrref{AGP} \DIFadd{term scaled by $\dotlambda$ is responsible for }\DIFaddend the non-adiabatic \DIFdelbegin \DIFdel{contribution generated by }\DIFdelend \DIFaddbegin \DIFadd{effects experienced by a system as it gets driven in a time-dependent fashion. From Eq.~\eqref{eq:AGP_diagonal}, we know that }\DIFaddend the \acrref{AGP} \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{operator can be expressed as
	}\begin{equation}
...
\cite{sels_minimizing_2017}}\hskip0pt%DIFAUXCMD
, and }\DIFaddend not only elucidates what we have talked about so far, but also gives some intuition for how we might overcome the practical problems associated with the exact \acrref{AGP}\DIFdelbegin \DIFdel{, setting }\DIFdelend \DIFaddbegin \DIFadd{. Furthermore, it sets }\DIFaddend the stage nicely for the rest of the chapter.

    The story goes something like this: imagine that you are a waiter tasked with carrying a glass of water on a tray from the bar to some table on the other side of a rather large restaurant. As you begin to walk\DIFaddbegin \DIFadd{, }\DIFaddend while holding the tray perfectly level with the ground, your acceleration induces a force on the glass which causes it to wobble and the water to splash around. Ideally you would like to stop the water from spilling, so at this point you have two options: either to (a) walk slowly \DIFaddbegin \DIFadd{enough }\DIFaddend so as to minimize the force that is destabilizing the glass or else (b) \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{suitably }\DIFaddend counteract it by\DIFaddbegin \DIFadd{, e.g.~}\DIFaddend tilting the tray. 

    You may already see where we are going with this. In the analogy, we can view the stable, upright state of the glass full of water as the ground state of some quantum system. The moving waiter then embodies the time-dependent Hamiltonian \DIFaddbegin \DIFadd{driving the system from this initial state}\DIFaddend , where we can model their changing coordinates as they move through the bar via the abstract parameter(s) $\lambda$. Just as in the case of the adiabatic condition of Eq.~\eqref{eq:adiabatic_criterion}, the \DIFdelbegin \DIFdel{chance }\DIFdelend \DIFaddbegin \DIFadd{probability }\DIFaddend of the glass tipping over depends on both the acceleration and direction of the waiter (the $\dlambda H$ term) as well as how inherently stable the glass is due to\@e.g.~a heavier bottom or more viscous liquid (the energy gap between the ground state and the nearest excited state). In this picture, the two methods the waiter can use to stabilize the glass during transport are analogous to (a) following the adiabatic condition by minimizing the speed at which the Hamiltonian is deformed or (b) applying \DIFdelbegin \DIFdel{counterdiabatic driving }%DIFDELCMD < \@i%%%
\DIFdel{.e.~counteracting }\DIFdelend \DIFaddbegin \DIFadd{some suitable control technique, such as counterdiabatic driving, to counteract }\DIFaddend the non-adiabatic force that appears as a consequence of their fast movement.

    This example is not only useful for gaining intuition about adiabaticity and \acrref{CD}, but can also be used to bring attention to several interesting observations. The first is that by including a counterdiabatic component, the waiter introduces a new degree of freedom - a tilt - which would otherwise not show up anywhere in the process or the start/end points of the journey of the glass. Secondly, from the point of view of someone standing by the wayside (the lab frame), the glass is nowhere near standing upright throughout the counterdiabatic tilt, rather it is in some highly excited state, while from the perspective of the glass (the moving frame) it is quite stable and generally close to the instantaneous ground state, as can be garnered from looking at Equations \eqref{eq:AGP_cancelling_out} and \eqref{eq:CD_Hamiltonian} \DIFdelbegin \DIFdel{as representative of }\DIFdelend \DIFaddbegin \DIFadd{which represent }\DIFaddend the two perspectives.
    \DIFaddbegin 
...
\DIFaddend The most important observation, however, which springboards us into the next section of this chapter, is precisely one which answers the question: \emph{how stable is the glass throughout the waiter's counterdiabatic journey}? We cannot assume, in any realistic scenario, that the waiter has perfect knowledge of the movement of every molecule of water in the glass and can control their movements to such high precision that they instantly counteract even the smallest deviation from the perceived ground state. In fact, it is far more likely that the waiter has very limited ability to tilt the tray as well as only the roughest, low-resolution model of the ways in which the glass wobbles. The result is that far from implementing an `exact' \acrref{CD} Hamiltonian as in Eq.~\eqref{eq:CD_Hamiltonian}, the waiter produces only some high-level approximation of the $\dotlambda \AGP{\lambda}$ term\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{. And }\DIFaddend yet, barring extreme circumstances, \DIFdelbegin \DIFdel{manages }\DIFdelend \DIFaddbegin \DIFadd{they manage }\DIFaddend to quickly and safely transport the glass from bar to table. 
    ...
    \begin{equation}\label{eq:energy_derivative}
        \DIFdelbegin %DIFDELCMD < \begin{aligned}
%DIFDELCMD <             \frac{dE}{dt} &= \frac{d}{dt}\ev*{\Tilde{H}}{\Tilde{\psi}} \\
%DIFDELCMD <             &= \mel*{\partial_t \Tilde{\psi}}{\Tilde{H}}{\Tilde{\psi}} + \mel*{\Tilde{\psi}}{\partial_t \Tilde{H}}{\Tilde{\psi}} + \mel*{\Tilde{\psi}}{\Tilde{H}}{\partial_t \Tilde{\psi}} \\
%DIFDELCMD <             &= \frac{i}{\hbar}\ev*{(\Tilde{H} - \dotlambda \Tilde{\AGP{\lambda}})\Tilde{H}}{\Tilde{\psi}} + \dotlambda \ev*{\dlambda \Tilde{H}}{\Tilde{\psi}} - \frac{i}{\hbar}\ev*{(\Tilde{H} - \dotlambda \Tilde{\AGP{\lambda}})\Tilde{H}}{\Tilde{\psi}}\\
%DIFDELCMD <             &= \dotlambda \ev*{\dlambda \Tilde{H}}{\Tilde{\psi}} - \frac{i}{\hbar} \dotlambda \ev*{\comm*{\Tilde{\AGP{\lambda}}}{\Tilde{H}}}{\Tilde{\psi}} \\
%DIFDELCMD <             \Rightarrow i\hbar F_{\lambda} &= \comm{\AGP{\lambda}}{H} - i \hbar \dlambda H,
%DIFDELCMD <         \end{aligned}%%%
\DIFdelend \DIFaddbegin \begin{aligned}
...
.
    ...
    \end{equation}
    where the coefficients $\alpha_k(\lambda)$ are used in a similar manner as \acrref{LCD}\DIFdelbegin \DIFdel{and can be found for each set of operators at order $k$ using the minimisation procedure outlined in section \ref{sec:2.4.1_LCD}}\DIFdelend \DIFaddbegin \DIFadd{. The minimisation procedure outlined in section \ref{sec:2.4.1_LCD} can be implemented to determine the coefficients $\alpha_k(\lambda)$ for all orders of the nested commutator expansion. On the other hand, one might choose to instead reparameterise and use a different set of coefficients: one $\alpha$ for each orthogonal operator that is obtained in the nested commutator expansion after a chosen number of commutations. The primary difference between the two approaches is merely the parameterisation of the approximate counterdiabatic drive. A more fine-grained parameterisation, with a larger number of coefficients $\alpha$, is liable to give a better approximation of the drive. In the original work \mbox{%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{While the nested commutator expansion does not appear in later chapters, it forms a backbone in the research on practical approximations of counterdiabatic protocols and was used extensively behind the scenes for many of the results presented in the thesis. As such, we found it prudent to include. For further reading on how one might combine the results from the rest of the thesis and the nested commutator method to great success, we refer the reader to \mbox{%DIFAUXCMD
\cite{lawrence_numerical_2024}}\hskip0pt%DIFAUXCMD
.
}

\DIFaddend \chapter{Quantum Optimal Control}\label{chap:3_Quantum_Optimal_control}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{images/optimal_control_illustration.png} \caption[Illustration of optimal control problem structure]{Illustration of the \DIFdelbeginFL \DIFdelFL{Mayer type }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Mayer-type }\DIFaddendFL optimal control problem: when an initial value of the system state $x_0$ is fixed, the choice of control function $u \in U$ and the requirement of satisfying Eq.~\eqref{eq:control_ODE} determine $x$ uniquely. The task is then to find $u_{\rm opt}$ such that the functional $C(x, u_{\rm opt})$ is minimised.}\label{fig:optimal_control}
\end{figure}

There are primarily three different types of problem structures in optimal control centering on different constraints and targets: \emph{Mayer-type}, \emph{Lagrange-type} and their combination, \emph{\DIFdelbegin \DIFdel{Bolza}\DIFdelend \DIFaddbegin \DIFadd{Bolza-type}\DIFaddend } problems \cite{dalessandro_introduction_2021}. In this thesis, we will mostly focus on Mayer-type problems, particularly in Ch.~\ref{chap:4_COLD} and Ch.~\ref{chap:6_Applications_fidelity}. In \DIFdelbegin \DIFdel{Mayer }\DIFdelend \DIFaddbegin \DIFadd{Mayer-type }\DIFaddend problems, the initial state is specified $x(0) = x_0$ and the cost function is of the form
\begin{equation}\label{eq:mayer_costfunc}
    C(u) = \phi(x(\tau), \tau),
\end{equation}
with $\phi$ a smooth function and $\tau$ the total time of the protocol. These two constraints and the requirement given by Eq.~\eqref{eq:control_ODE} define the state function $x$ uniquely and the problem is then to determine a control function $u$ on the appropriate set $[0, \tau]$ which minimises Eq.~\eqref{eq:mayer_costfunc}. \DIFaddbegin \DIFadd{We note that the expression in Eq.~\eqref{eq:mayer_costfunc} is quite general and can include multiple types of `constraints', e.g.~as a linear superposition. }\DIFaddend In Mayer-type problems, a specific target state can be defined in the cost function as a constraint, which is the case in Eq.~\eqref{eq:example_cost_func} and this is illustrated in Fig.~\ref{fig:optimal_control}. However, this need not be the case as target states can be made implicit by having the cost function target some property of the state instead, like Euclidean distance from the initial state in the case of real vectors over Cartesian coordinates. 
...
Analytical optimal control techniques are those that leverage mathematical rigor and formalism to derive solutions or insights, as opposed to relying primarily on numerical simulations, heuristics, or experimentation. They provide a theoretical foundation for understanding the properties and solutions of optimal control problems and are closely related to the discussion in Sec.~\ref{sec:3.1.1_mathematical_structure}. They can allow for a complete geometric understanding of the control problem leading to, for example, knowledge of the structure of a solution or even some proof about a global optimum. For a given set of constraints they might even be used to derive time limits of state transformations, \@i.e.~the concept of reachability. An example of analytical methods is the aforementioned \acrref{PMP}, which provides information about the optimal solution \DIFdelbegin \DIFdel{by }\DIFdelend via a set of differential equations. A different analytical control theory tool, the Hamilton-Jacobi-Bellman equation \cite{yong_dynamic_1999}, provides a way to find the optimal protocol via dynamic programming\cite{sniedovich_dynamic_2010}. \DIFaddbegin \DIFadd{We note here that the concepts introduced in the previous chapter may be viewed as a form of analytical optimisation - computation of the }\acrref{AGP} \DIFadd{(Sec.~\ref{sec:2.2_AGP}) analytically provides information about the non-adiabatic effects a system experiences given a certain path through the Hamiltonian parameter space and }\acrref{CD} \DIFadd{(Sec.~\ref{sec:2.3_CD}) then provides an optimal control protocol that drives a system via a desired trajectory. The cost function can be viewed as some measure of the magnitude of non-adiabatic effects generated by the }\acrref{AGP} \DIFadd{-- something we will explore in more detail in Ch.~\ref{chap:5_cd_as_costfunc}.
}\DIFaddend 

The trouble with analytical approaches, despite the commonplace rigorous guarantees of optimality and the scope of information they provide about the system, trajectory and structure of the control problems and their solutions, is that they are very difficult to scale up and quite inflexible to complex problem constraints. \DIFaddbegin \DIFadd{Once again, the concept of }\acrref{CD} \DIFadd{from the last chapter provides and excellent example of this problem, since an exact counterdiabatic drive may get exponentially more difficult to compute as the system scale or complexity increases. }\DIFaddend As such, analytical approaches are generally reserved for special cases, when problems have low dimensionality and simple structures \DIFdelbegin \DIFdel{where the cost function }\DIFdelend \DIFaddbegin \DIFadd{with a cost function that }\DIFaddend is generally linear in the arguments. Many real-world control systems require more complexity and flexibility than can be afforded by analytical methods.
...
\end{equation}
where $n$ denotes the current iteration of the algorithm, $\grad_{\ubb} C(\ubb_n)$ is the derivative of the cost function $C$ with respect to the control parameters $\ubb$ and $\mu$ is generally known as the `learning rate' or `step size' and its job is to control the resolution at which the algorithm traverses the cost landscape. Larger $\mu$ might lead to faster convergence but it might also mean overshooting the cost function minimum, so adjusting its value is often a heuristic that requires some experimentation. \DIFaddbegin \DIFadd{One way that many popular optimisers, such as ADAM \mbox{%DIFAUXCMD
\cite{kingma_adam_2015}}\hskip0pt%DIFAUXCMD
, overcome this issue is by implementing a variable, adaptive value of $\mu$ throughout the optimisation process. }\DIFaddend Other examples of gradient-based methods include Newton's method and quasi-Newton methods \cite{suli_introduction_2003}, which employ information about the second derivative to guide the search and provide faster convergence as well as a myriad of other approaches including stochastic methods \cite{bottou_tradeoffs_2007}. 
...
\begin{figure}[t]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.6\linewidth]{images/nelder_mead_illustration.png} %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\linewidth]{images/nelder_mead_illustration.png} \DIFaddendFL \caption[Visualising the Nelder-Mead optimisation algorithm.]{Illustration of the Nelder-Mead algorithm for a cost function parameterised by two parameters $\beta_1$ and $\beta_2$.  The simplex for a 2-dimensional landscape is a triangle \DIFdelbeginFL \DIFdelFL{, }\DIFdelendFL which \DIFdelbeginFL \DIFdelFL{then follows }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{transforms continuously seeking out a lower cost function value in }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{rough algorithm }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{landscape as }\DIFaddendFL described in the \DIFaddbeginFL \DIFaddFL{main }\DIFaddendFL text. \DIFdelbeginFL \DIFdelFL{The iterations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(a) Examples }\DIFaddendFL of \DIFaddbeginFL \DIFaddFL{three of }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{algorithm are indicated by constantly switching from solid }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{transformations that the simplex performs during the optimisation: 1. expansion, 2. reflection and 3. contraction. Solid lines indicate the simplex prior }\DIFaddendFL to \DIFaddbeginFL \DIFaddFL{the transformation while }\DIFaddendFL dashed \DIFdelbeginFL \DIFdelFL{edges }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{lines indicate its state after. (b) A plot }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{the simplex at each }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{every third }\DIFaddendFL step \DIFdelbeginFL \DIFdelFL{as well as }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of the algorithm, with the }\DIFaddendFL changing \DIFdelbeginFL \DIFdelFL{in colour}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{color of the triangle, starting from red and ending with bright purple, indicating the iterations}\DIFaddendFL . The inset shows a magnification of the final steps of the algorithm. \DIFaddbeginFL \DIFaddFL{The cost function landscape is represented by the contour plot and the aim of the Nelder-Mead algorithm in this case is to find its local minimal value (dark purple, as indicated by the colorbar).}\DIFaddendFL }\label{fig:nelder_mead}
\end{figure}
...
The vertices of this simplex then traverse the cost function landscape according to the Nelder-Mead algorithm in order to converge to some minimum value.  In most of the search steps, the primary change is to shift the highest point of the simplex (\@i.e.~where the cost function value is largest) through the opposite face of the simplex, moving to a point with a lower cost function value. These steps are known as \emph{reflections} and they are designed to preserve the volume of the simplex, ensuring it remains non-degenerate. Whenever possible, the method will \DIFdelbegin \DIFdel{expand }\DIFdelend \DIFaddbegin \emph{\DIFadd{expand}} \DIFaddend the simplex along a particular direction, which allows it to take bigger steps in search of a minimum. When the simplex encounters a region that can be thought of as a `valley floor' in the cost function landscape, it \DIFdelbegin \DIFdel{reduces }\DIFdelend \DIFaddbegin \emph{\DIFadd{contracts}} \DIFaddend its dimensions orthogonal to the valley, so that it can slide down. \DIFaddbegin \DIFadd{See Fig.~\ref{fig:nelder_mead} (a) for a visual reference. }\DIFaddend In situations where the simplex has to navigate through a narrow passage, it shrinks itself in all directions, wrapping itself around its best (lowest) point, enabling it to continue its search for the minimum. \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{The whole }\DIFaddend process is illustrated for a simple example in Fig.~\ref{fig:nelder_mead} \DIFaddbegin \DIFadd{(b)}\DIFaddend .

This description of the Nelder-Mead method only outlines the basic idea that was first developed in the original 1965 paper. Many variations and improvements have been developed in the years since and the actual implementations \DIFaddbegin \DIFadd{vary}\DIFaddend . In general, the Nelder-Mead approach is simple to understand and implement, as well as being quite efficient and flexible. However, it often suffers from convergence issues, being both likely to return a sub-optimal local minimum and to get stuck without converging far longer than necessary, undoing any efficiency it otherwise promised. Furthermore, the simplex method doesn't scale well in higher dimensions, making it less effective \DIFdelbegin \DIFdel{when }\DIFdelend \DIFaddbegin \DIFadd{as }\DIFaddend the number of parameters \DIFdelbegin \DIFdel{is large}\DIFdelend \DIFaddbegin \DIFadd{increases}\DIFaddend .

\subsubsection{Powell's method}\label{sec:3.1.3.2_Powell}

Another approach from the gradient-free, local optimiser crowd is Powell's method, first developed by Michael J. D. Powell in 1964 \cite{powell_efficient_1964}. The algorithm is known as a \emph{conjugate-direction} approach, not to be confused with the more common conjugate-gradient approach \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{hestenes_methods_1952}}\hskip0pt%DIFAUXCMD
}\DIFaddend , although the two are related as the latter can be viewed as a specialisation of the former. 

The basis of Powell's method relies on the idea of conjugate vectors or conjugate directions. Two vectors $\ubb$ and $\boldsymbol{v}$ are said to be conjugate with respect to some positive \DIFdelbegin \DIFdel{semi-definite }\DIFdelend \DIFaddbegin \DIFadd{semidefinite }\DIFaddend matrix $A$ if $\ubb^T A \boldsymbol{v} = 0$. A set of conjugate directions, thus, is a set of vectors that are pairwise conjugate. Furthermore, one can make the observation \cite{brent_algorithms_2002} that the function
\begin{equation}
    f(\xbb) = \xbb^T A \xbb - 2\boldsymbol{b}^T \xbb + c
\end{equation}
for some positive semidefinite matrix $A$, $\boldsymbol{b} \in \R^n$ and $c \in \R$ has a minimum at the point \DIFdelbegin \DIFdel{$\sum_{i=1}^n \beta_i \ubb_i$ }\DIFdelend \DIFaddbegin \DIFadd{$\xbb = \sum_{i=1}^n \beta_i \ubb_i$ }\DIFaddend in the space spanned by the set of conjugate vectors $\{ u_j \}_{j = 1,...,n}$ with
\begin{equation}
...
More concretely, at each step of the search, the algorithm generates a trial jump in the cost function space from the current temporary solution to a new point\DIFdelbegin \DIFdel{by sampling it }\DIFdelend \DIFaddbegin \DIFadd{. This is done by sampling }\DIFaddend from a modified Cauchy-Lorentz distribution \DIFaddbegin \DIFadd{over the cost function space}\DIFaddend . The distribution peaks around the current temporary solution and its scale parameter (a variable that controls its spread) is a function of the artificial temperature $T_{q_v}$. Thus, the higher the temperature, the more likely it is that the trial jump will be larger\DIFaddbegin \DIFadd{, taking the solver further away from its current location in the cost function space}\DIFaddend . The $q_v$ parameter can be set to different values in order to speed up or slow down the cooling process. 

Once the trial jump has been generated, it is either accepted or rejected based on the cost function value at the new point as compared to the current point. If the new point is better (\@i.e.~the jump is `downhill', towards a lower energy), then the jump is accepted. If, on the other hand, the jump is worse or `uphill', it might still be accepted with some probability based on a parameterised Metropolis algorithm \cite{chib_understanding_1995}\DIFaddbegin \DIFadd{, where the probability of acceptance is calculated roughly as
}\begin{equation}
	\DIFadd{P = \exp{-\Delta C / T_{q_v}},
}\end{equation}
\DIFadd{with $\Delta C$ the change in the cost function value from the previous trial solution to the new one and $T_{q_v}$ the artificial temperature}\DIFaddend . This allows for the algorithm to potentially escape local minima. If a jump is accepted, the search then continues in a similar manner from the new point and the temperature parameter is decreased, reducing the probability of the next generated jump being far away from the current point \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{and the potential of accepting a jump to a `worse' value. Once the temperature reaches 0, the system `freezes' and can only transition to states with lower `energy' or cost function value in its immediate vicinity until it reaches a local minimum.
}\DIFaddend 
...
The verdict regarding dual-annealing, with respect to the local optimisers that we addressed previously, is that it is \DIFdelbegin \DIFdel{far }\DIFdelend more powerful and can lead to \DIFdelbegin \DIFdel{far }\DIFdelend better solutions, given that it has a \DIFdelbegin \DIFdel{far }\DIFdelend better ability to explore the cost function landscape. \DIFaddbegin \DIFadd{Many real-world cost function landscapes are non-convex, high-dimensional and consist of many local minima. In such cases local optimisers, by virtue of only searching locally, will always get trapped in a local minimum if initialised near one. If there are many such local minima, then a local optimiser is highly likely to consistently not find the global optimum/minimum. A global optimiser like dual-annealing, on the other hand, may be initialised anywhere on the landscape and will hop around the entire solution space due to its stochastic nature, ignoring locally good solutions in order to occasionally move `up the hill'. This allows it to converge to a global optimum rather than a local one with far higher certainty than any local optimiser.
}

\DIFaddend However, \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{dual-annealing }\DIFaddend is also more computationally expensive \DIFaddbegin \DIFadd{than local methods}\DIFaddend , as can be made obvious by the fact that local search is merely a subroutine of the algorithm. \DIFaddbegin \DIFadd{The constant hopping around the landscape generally requires far more iterations than an algorithm like Nelder-Mead. }\DIFaddend Ultimately, the choice of which approach \DIFaddbegin \DIFadd{to use }\DIFaddend comes down to having knowledge about the cost function landscape as well as trial-and-error. The use of a global optimiser may be overkill when the cost function landscape lends itself well to local methods \DIFaddbegin \DIFadd{(e.g.~when it is highly convex) }\DIFaddend and each evaluation of the cost function is expensive. If, however, \DIFdelbegin \DIFdel{local }\DIFdelend \DIFaddbegin \DIFadd{locally }\DIFaddend optimal solutions are not enough, then global methods are by far the best option.

\section{Quantum optimal control}\label{sec:3.2_Quantum_optimal_control}

We've now established that the broad goal of optimal control theory is the design of protocols and strategies which optimise the behaviour of some abstract control system with respect to some abstract target. Quantum optimal control theory (\acrref{QOCT}), rather predictably, does this in the setting where the abstract system is a quantum system. Very broadly then, \acrref{QOCT} concerns itself with the design and analysis of \DIFdelbegin \DIFdel{electromagnetic fields }\DIFdelend \DIFaddbegin \DIFadd{control fields (usually electromagnetic fields) }\DIFaddend that manipulate quantum dynamical processes at the atomic or molecular scale in the best way possible, as illustrated in Fig.~\ref{fig:quantum_optimal_control}. In this chapter, we will broadly cover the basics of \acrref{QOCT}, starting with how the mathematical structure discussed in Sec.~\ref{sec:3.1.1_mathematical_structure} can be adapted to the quantum setting and ending with detailed descriptions of \acrref{CRAB} (Sec.~\ref{sec:3.3.1_CRAB}) and \acrref{GRAPE} (Sec.~\ref{sec:3.3.2_GRAPE}), popular \acrref{QOCT} methods which will be relevant to later work presented in this thesis. As the content of later chapters will focus on closed systems, that will be the perspective we will take with respect to \acrref{QOCT}. More concretely, we will focus on cases where the generator of transformations of a quantum system is primarily modelled as the Hamiltonian as opposed to, \@e.g.~a Liouvillian, but a similar\DIFaddbegin \DIFadd{, if generally more complex, }\DIFaddend analysis holds in the case of open systems\DIFdelbegin \DIFdel{which are just as interesting, if not quite as relevant to this thesis}\DIFdelend .  

Returning to the material covered in Sec.~\ref{sec:3.1.1_mathematical_structure}, we can now add more structure to the abstract notions of system, control function and cost function. In the quantum setting, the set of state functions $X$ often takes the form of a set of quantum states, be they complex vectors, density matrices or operators. The set of control functions $U$ is usually represented by a set of functions of parameterised Hamiltonians. It is common to decompose a control Hamiltonian into two components: the time-dependent `drive' part and the time-independent `drift' part. The time-dependent part can then be further decomposed into a set of $N_k$ operators \DIFdelbegin \DIFdel{$\{\mathcal{O}_{\rm opt}^{(k)}\}_{k=1,...,N_k}$}\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{O}_{\rm opt} = \{\mathcal{O}_{\rm opt}^{(k)}\}_{k=1,...,N_k}$}\DIFaddend , such that the full control Hamiltonian reads:
\begin{equation}\label{eq:optimal_control_H}
    H(\ubb(t)) = H_0 + \sum_{k = 1}^{N_k} u_k(t) \mathcal{O}_{\rm opt}^{(k)},
\end{equation}
where $H_0$ is the \DIFdelbegin \DIFdel{drift Hamiltonian with no external controls and the }\DIFdelend \DIFaddbegin \DIFadd{so-called `drift' Hamiltonian which drives the free evolution of the system and the additional terms made up of }\DIFaddend control functions $u_k(t) \in \ubb(t)$ \DIFdelbegin \DIFdel{drive }\DIFdelend \DIFaddbegin \DIFadd{driving }\DIFaddend the corresponding operators $\mathcal{O}_{\rm opt}^{(k)}$.
...
\end{equation}
where the cost function landscape is spanned by the coefficients $c_i$, that need to be optimised over using numerical optimisation methods like those described in Sec.~\ref{sec:3.1.3_numerical_optimisation}. Generally this basis is made up of trigonometric functions\DIFaddbegin \DIFadd{, since their behaviour is considered easy to understand, }\DIFaddend although it could be any basis that spans the space of admissible controls \@e.g.~\DIFaddbegin \DIFadd{something like the }\DIFaddend generalized Chebyshev polynomials. A choice of basis can further be enhanced or modified by a shape function $g(t)$ that fixes the pulse to some initial and/or final value:
...
\begin{equation}\label{eq:COLD_Hamiltonian}
    ...
    \end{aligned}
\end{equation}
which is the \acrref{COLD} Hamiltonian. \DIFaddbegin \DIFadd{Note that if the set $\mathcal{O}_{\rm opt}$ is not a subset of $\mathcal{O}_{\rm H}$, then the operators in $\mathcal{O}_{\rm CD}$ and consequently $\mathcal{O}_{\rm LCD}$ may be different than in the case when only driving the bare Hamiltonian. 
}\DIFaddend 
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.8\linewidth]{images/COLD_illustration.png} \caption[A diagrammatic illustration of the COLD method]{A diagrammatic illustration of the COLD method. \DIFaddbeginFL \DIFaddFL{(left) }\DIFaddendFL The \DIFdelbeginFL \DIFdelFL{blue shape is the }\DIFdelendFL set of control Hamiltonians with control parameters $\betabb$, with each point within the shape representing a different instance of $\betabb$. In the case where the control amplitude is $0$ throughout the evolution, we recover the bare Hamiltonian $H(\lambda, \hbb)$ (red point). \DIFaddbeginFL \DIFaddFL{(right) }\DIFaddendFL The \DIFdelbeginFL \DIFdelFL{green shape represents the }\DIFdelendFL system state space with $\ket{\psi_0}$ the eigenstate of $H(\lambda_0)$ that the system is prepared in and $\ket{\psi_f}$ the corresponding eigenstate of $H(\lambda_f)$, which is the target. The dotted (no \acrref{LCD} drive) and chalky (\acrref{LCD} drive included) directed lines show how the system is driven across state space for a fixed total time $\tau$ in the case of each control Hamiltonian indicated on the blue shape. \acrref{COLD} essentially allows one to use optimisation in order to find a path via the value of $\betabb$ which will lead to the result which is closest to the final state when \acrref{LCD} is applied, \@e.g.~going from the red path to the purple. \DIFaddbeginFL \DIFaddFL{Note that in cases where }\acrref{LCD} \DIFaddFL{is equivalent to the exact }\acrref{CD} \DIFaddFL{the arrows in the state space will lead from the initial state to the final state exactly, although the control parameter might change the shape of the path that they take to get there.}\DIFaddendFL }\label{fig:COLD_illustration}
\end{figure}
...
These three methods are by no means the only way to construct optimal pulses for \acrref{COLD} and, as discussed in Sec.~\ref{sec:3.2_Quantum_optimal_control}, the field of quantum optimal control is vast \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{koch_quantum_2022}}\hskip0pt%DIFAUXCMD
...
Returning to the key ideas behind \acrref{CD}, we may recall that the exact \acrref{CD} pulse is comprised of the \acrref{AGP} \DIFdelbegin \DIFdel{(Sec.~\ref{sec:2.2_AGP}) }\DIFdelend operator $\AGP{\lambda}$ scaled by the rate of change of $\lambda(t)$ (expressed as $\dotlambda$) in the adiabatic Hamiltonian, as given by Eq.~\eqref{eq:CD_Hamiltonian}. The \acrref{AGP} is the generator of adiabatic deformations between quantum eigenstates, and its off-diagonal elements are responsible for transitions between the instantaneous (or adiabatic) eigenstates\DIFdelbegin \DIFdel{or, put }\DIFdelend \DIFaddbegin \DIFadd{. Put }\DIFaddend another way, the Frobenius norm of the \acrref{AGP} is the distance between nearby \DIFaddbegin \DIFadd{adiabatic }\DIFaddend eigenstates \cite{pandey_adiabatic_2020, nandy_delayed_2022}. Thus, there are two components comprising the non-adiabatic effects experienced by a system driven at finite time by a time-dependent Hamiltonian: the rate of change of the parameters $\dotlambda$ and the \acrref{AGP} operator, which can be quantified \@e.g.~via \DIFdelbegin \DIFdel{its norm }\DIFdelend \DIFaddbegin \DIFadd{the magnitude of $\dotlambda$ or the norm of the adiabatic gauge potential}\DIFaddend . 

In the design \DIFaddbegin \DIFadd{of }\DIFaddend fast adiabatic protocols such as the techniques under the umbrella of \acrref{STA}, we are competing against $\dotlambda$\DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{, since }\DIFaddend the aim in general is speed rather than adherence to the adiabatic condition. This leaves us with minimising $\AGP{\lambda}$\DIFaddbegin \DIFadd{, }\DIFaddend or else, as in the case of \acrref{CD}, suppressing or mitigating its effects. To whit, \acrref{LCD} does this by implementing an operator that approximately suppresses the \acrref{AGP} for a given Hamiltonian. \acrref{COLD} then aids in this endeavour by modifying the Hamiltonian and thus the corresponding \acrref{AGP} operator in a way that allows for a given \acrref{LCD} protocol to perform better. The optimal control component of \acrref{COLD} in \cite{cepaite_counterdiabatic_2023} and throughout Ch.~\ref{chap:6_Applications_fidelity} is implemented with the target state fidelity (Eq.~\eqref{eq:costfunc_fidelity}) as a cost function, which is \DIFdelbegin \DIFdel{down }\DIFdelend \DIFaddbegin \DIFadd{due }\DIFaddend to the fact that the primary application of adiabatic protocols in often (though not always \cite{pelegri_high-fidelity_2022}) state preparation. The use of fidelity as a cost function, however, necessitates access to the wavefunction of the final \DIFaddbegin \DIFadd{prepared }\DIFaddend state, something that becomes difficult \DIFdelbegin \DIFdel{to compute for }\DIFdelend \DIFaddbegin \DIFadd{in the case of }\DIFaddend large or highly correlated systems\DIFaddbegin \DIFadd{, }\DIFaddend and may result in a highly non-convex or complex cost function landscape (see, for example, Fig.~\ref{fig:ghz_contours}). Furthermore, fidelity is not a useful cost function in practice\DIFaddbegin \DIFadd{, particularly }\DIFaddend in the case where the target state is unknown, which is common in \@e.g.~applications of adiabatic protocols to Hamiltonians whose ground states encode solutions to combinatorics problems \cite{ebadi_quantum_2022, albash_adiabatic_2018}. 

A solution to the problem of fidelity as a cost function might be the use of a different metric for optimising the Hamiltonian path. We have established that $\AGP{\lambda}$ contains information pertaining to non-adiabatic losses experienced by a driven system\DIFdelbegin \DIFdel{, thus }\DIFdelend \DIFaddbegin \DIFadd{. Thus, }\DIFaddend a natural approach to optimising the path of the Hamiltonian would be to minimise the \acrref{AGP} operator (we will discuss the details of this in the next section), as this should in principle minimise losses associated with non-adiabaticity. The main advantages of such an approach would be the fact that  the minimisation should be far more efficient than any attempt to compute the full system evolution, assuming we have access to the \acrref{AGP} as a function of the Hamiltonian path in parameter space. \DIFaddbegin \DIFadd{In the case of }\acrref{LCD}\DIFadd{, this is a valid assumption. }\DIFaddend Furthermore, this process would require no knowledge of the system wavefunction at any point, removing the drawbacks discussed earlier concerning the \DIFaddbegin \DIFadd{efficiency of implementing the }\DIFaddend fidelity cost function.
...
Intuitively, if the integral cost function is $0$, then this second approach does not provide any more information as the two will be equivalent. However, what this cost function captures is whether or not the path of the system for a given $\betabb$ and $\tau$ experiences any critical point in its evolution where the non-adiabatic effects are maximised, for example in the case of closing gaps between the instantaneous eigenstates. This cost function may be useful in cases where one wants to avoid such closing gaps within the system evolution and no path optimally suppresses all non-adiabatic effects. In particular, it might be used in \acrref{LCD} as a means to avoid cases where the \acrref{CD} operators \DIFdelbegin \DIFdel{which are not applied are }\DIFdelend \DIFaddbegin \emph{\DIFadd{which are not countered via a counterdiabatic drive}} \DIFadd{are }\DIFaddend responsible for most of the avoided transitions out of the instanteous eigenstate. \DIFaddbegin \DIFadd{For example, should one only be able to apply a counterdiabatic drive to a single subsystem of many, then it would make sense to aim to create a driving protocol for the full system which minimises the non-adiabatic effects experienced by every other system that cannot have them be suppressed in such a way. In other words, if you can't counter the losses, find a way to not generate them in the first place. 
}\DIFaddend \part{Applications of COLD}\label{part:applications}
...
\end{equation}
where the operator subscripts denote the index of the spin on which they act, $H_0$ is parameterised by the functions $\hbb = \{J(\lambda), Z(\lambda), X(\lambda)\}$, with the $\lambda(t) = \frac{t}{\tau}$ term encoding the time-dependence and where $J(\lambda) = -2J_0$ and $z(\lambda) = -h_0$ are constant functions. \DIFaddbegin \DIFadd{The added transverse field allows us to explore a larger operator basis for the }\acrref{LCD}\DIFadd{. }\DIFaddend For this example we use
\begin{equation}\label{eq:lambda_func1}
X(\lambda) = 2 h_0 \sin^2\left(\frac{\pi}{2} \sin^2 \left( \frac{\pi}{2} \lambda \right) \right),
\end{equation}
where we note that since $\lambda(0) = 0$ and $\lambda(\tau) = 1$, the transverse field is tuned from $0$ to $2h_0$ as $t$ goes from $0$ to $\tau$. \DIFaddbegin \DIFadd{The choice of function is rather arbitrary, but in general we want driving functions which are smooth, in particular near the beginning and end of the protocol, since they lead to a more smooth,  well-behaved counterdiabatic term which tends to 0 at the beginning and end of the driving time without any sudden jumps in value, making its experimental implementation more amenable. This is a consequence of the }\acrref{LCD} \DIFadd{drive's dependence on the derivatives of the functions in set $\hbb$. Fig.~\ref{fig:lcd_drives} illustrates the difference in the resulting }\acrref{LCD} \DIFadd{drives when using the smooth version of $X(\lambda)$ given above and a linear protocol. We can see that in the case of a linearly increasing drive, the }\acrref{LCD} \DIFadd{driving terms are generally non-zero at the start and end of the protocol and may exhibit sudden jumps in value due to the larger derivative of the driving function at certain instances during the driving procedure. 
}

\DIFaddend We consider the case where $J_0/h_0 = 0.5$, meaning that the initial ground state of the system is in the $\ket{\uparrow \uparrow}$ state and the ground state at $t = \tau$ should be a superposition of all the symmetric states. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend As per the discussion in \DIFdelbegin \DIFdel{in }\DIFdelend Sec.~\ref{sec:2.4.1_LCD}, since $H_0$ is a real-valued Hamiltonian, the single-spin LCD operators should be fully imaginary and thus given by the following ansatz for the adiabatic gauge potential:
\begin{equation}\label{eq:LCD1st}
\approxAGP^{(1)}(\lambda, \hbb) = \alpha(\lambda, \hbb) (\sy_1 + \sy_2),
\end{equation}
which we will indicate as `first-order' or \acrref{FO} \acrref{LCD}, referring to the fact that these are the most local spin operators and denoting this fact with the superscript $(1)$. In this case, we \DIFdelbegin \DIFdel{expect }\DIFdelend \DIFaddbegin \DIFadd{know that }\DIFaddend the \acrref{LCD} pulse \DIFdelbegin \DIFdel{to be the same }\DIFdelend \DIFaddbegin \DIFadd{is of the same form }\DIFaddend for both $\sy_1$ and $\sy_2$ due to the symmetry of the Hamiltonian\DIFdelbegin \DIFdel{, hence only requiring }\DIFdelend \DIFaddbegin \DIFadd{. Thus, we only require }\DIFaddend one coefficient $\alpha$ for both operators. In future sections where this is not the case, we will instead differentiate between global and local \acrref{LCD} pulses\DIFdelbegin \DIFdel{, where }\DIFdelend \DIFaddbegin \DIFadd{. In the case of local }\acrref{LCD}\DIFadd{, }\DIFaddend unique pulses may be required for each separate operator \DIFdelbegin \DIFdel{, but due to physical constraints we }\DIFdelend \DIFaddbegin \DIFadd{to capture the counterdiabatic effects more faithfully, but for the sake of simplicity and the commonplace difficulty of highly precise local control of systems in expreiment, we will generally }\DIFaddend approximate them with a single coefficient.

Using the methods described in \cite{sels_minimizing_2017} and summarised in Sec.~\ref{sec:2.4.1_LCD}, we can determine the form of the coefficient $\alpha$ using a variational approach, which we will set out in detail here to illustrate the method. For the given $H_0$, and $\approxAGP$, the first step is to find the operator $G_{\lambda}$ from Eq.~\DIFdelbegin \DIFdel{\ref{eq:G_operator}:
}\DIFdelend \DIFaddbegin \DIFadd{\eqref{eq:G_operator} (setting, in true Physics fashion, $\hbar = 1$):
}\DIFaddend \begin{equation}
    \DIFdelbegin %DIFDELCMD < \begin{aligned}
%DIFDELCMD <         G_{\lambda}(\approxAGP^{(1)}, H_0) &= \dlambda H_0 + \frac{i}{\hbar} \comm{\approxAGP^{(1)}}{H_0} \\
%DIFDELCMD <         &= \dlambda {X} (\sx_{1} + \sx_{2}) - 2 \alpha J (\sx_1\sz_2 + \sz_1\sx_2) - 2 \alpha Z (\sx_{1} + \sx_{2}) \\
%DIFDELCMD <         &+ 2 \alpha X (\sz_{1} + \sz_{2}),
%DIFDELCMD <     \end{aligned}%%%
\DIFdelend \DIFaddbegin \begin{aligned}
    ...
    \end{aligned}\DIFaddend 
\end{equation}
where the $\lambda$-dependence is omitted and the only non-constant function is $X(\lambda)$, making it the only \DIFdelbegin \DIFdel{non-zerp }\DIFdelend \DIFaddbegin \DIFadd{non-zero }\DIFaddend contribution to $\dlambda H_0$. We then obtain the action from Eq.~\eqref{eq:agp_action} defined as $\mathcal{S} = \Tr[G^2_{\lambda}]$:
\begin{equation}
...
\end{equation}
for the Hamiltonian $H_0(\hbb, \lambda)$\DIFdelbegin \DIFdel{, given any }\DIFdelend \DIFaddbegin \DIFadd{. Given any other }\DIFaddend $\hbb$ and $\lambda$\DIFaddbegin \DIFadd{, the equations above can be modified quite easily by changing the form of the action correspondingly}\DIFaddend . 
...
\end{equation}
The results are computed for the case where only the bare Hamiltonian $H_0(\hbb, \lambda)$ from Eq.~\DIFdelbegin \DIFdel{\ref{eq:two_spin_hamiltonian} }\DIFdelend \DIFaddbegin \DIFadd{\eqref{eq:two_spin_hamiltonian} }\DIFaddend drives the system and they are then compared to \acrref{FO} \acrref{LCD} (Eq.~\eqref{eq:FO_LCD_H}), \acrref{SO} \acrref{LCD} (the solution to Eq.~\eqref{eq:two_spin_coupled_eqs} with $\alpha$ set to $0$) and the exact counterdiabatic drive (Eq.~\eqref{eq:two_spin_exact_AGP}). The results show that at fast evolution times ($\tau < 1 h_0^{-1}$) when no counterdiabatic drive is applied the final state remains far from the ground state of $H_0(\lambda = 1)$ and only begins to approach the target as the evolution time is increased, exactly as one might expect given the adiabatic condition (Sec.~\ref{sec:2.1.2_adiabatic_condition}). In the case of both \acrref{FO} and \acrref{SO} \acrref{LCD}, however, the system approaches the desired ground state with close to unit fidelity even at very short driving times several orders of magnitude faster. As expected, when the exact \acrref{CD} is applied including all single- and two-spin imaginary operators, the system reaches the desired state with unit fidelity at arbitrarily short driving times, as in the rotating spin example we discussed in Ch.~\ref{chap:2_adiabaticity} and Appendix~\ref{app:rotating_spin_hamiltonian}. \DIFaddbegin \DIFadd{There is a curious phenomenon, wherein for all pulses barring exact }\acrref{CD} \DIFadd{(}\acrref{FO} \DIFadd{+ }\acrref{SO} \acrref{LCD}\DIFadd{), the system fidelity is flat for some time before rising to 1 at around the same time. A natural interpretation of this is that around driving time $\tau = 1 h_0^{-1}$ the system enters the adiabatic regime. Prior to this point, it is likely that in each of the different cases (}\acrref{BPO}\DIFadd{, }\acrref{FO} \DIFadd{and }\acrref{SO} \acrref{LCD}\DIFadd{), there is a different fundamental limit to the controllability of the system that depends on what types of external drives are applied, as the fidelity plateaus at different values for each method regardless of driving time. For a more in-depth discussion on how this might relate to the quantum speed limit and the way that the }\acrref{LCD} \DIFadd{affects the spectrum of the Hamiltonian, we refer the reader to Ref.~\mbox{%DIFAUXCMD
\cite{abah_energetic_2019}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
\DIFaddbegin \begin{figure}[t]
    \centering
    \includegraphics[width=0.8\linewidth]{images/lcd_drive_plots} \caption[Driving strengths of LCD for different Hamiltonian protocols.]{\DIFaddFL{Figure illustrating the functional forms of the drives in the two-spin annealing protocol of Eq.~\eqref{eq:two_spin_hamiltonian}. The dark blue plots indicate the functional form of $X(\lambda)$ scaled by $(2h_0)^{-1}$ (such that $X(\lambda) = 2 h_0 f(\lambda)$ where $f(\lambda)$ is the function being plotted). The solid plots are for the case given in Eq.~\eqref{eq:lambda_func1}, where $f(\lambda) = \sin^2\left(\frac{\pi}{2} \sin^2 \left( \frac{\pi}{2} \lambda \right) \right)$, while the dashed plots describe the case where a linear drive is used instead: $f(\lambda) = \lambda$, remembering that $\lambda = \frac{t}{\tau}$. Each set of axes above shows the resulting drives for different choices of }\acrref{LCD} \DIFaddFL{ansatz, as indicated by the inset equations. Note that the case where $\approxAGP = \gamma(\sz_1\sy_2 + \sy_1\sz_2)$, the resulting drive is zero throughout the protocol and hence it is not plotted above.}}\label{fig:lcd_drives}
\end{figure}

\DIFaddend As the goal of \acrref{COLD} is to improve \acrref{LCD} in restricted settings, the most natural approach to demonstrate it in a simple example like this is to take the \acrref{FO} \acrref{LCD} ansatz as a fixed approximation for the counterdiabatic drive and to ignore \acrref{SO} terms. This is a realistic scenario, as even two-spin operators like those found in the \acrref{SO} \acrref{LCD} anstaz of Eq.~\eqref{eq:twospin_so_lcd} are generally difficult to engineer in physical systems and thus even in this simple case it is unlikely that the exact \acrref{CD} pulse could be implemented, though its functional form is known.

Revisiting Ch.~\ref{chap:4_COLD}, we find that the first step of \acrref{COLD} is to construct a control pulse for the Hamiltonian. Ideally, this is something that can be easily controlled in an experimental setting and for the given $H_0$, we can imagine introducing a $\lambda$-dependent control for the coupling, transverse or longitudinal fields. For simplicity and pedagogy, we can introduce a `bare' pulse (Sec.~\ref{sec:4.2_COLD_QOCT}) control component to the Hamiltonian which drives the $(\sz_1 + \sz_2)$ operators \DIFaddbegin \DIFadd{(the choice is arbitrary - we may as well have picked  $(\sx_1 + \sx_2)$ or any other operator form the available degrees of freedom - though the results would change depending on that choice) }\DIFaddend and obeys the constraint of being $0$ at the beginning and end of the driving time. This gives a control Hamiltonian:
...
In Fig.~\ref{fig:ising_unconstrained}(a) we plot the results \DIFaddbegin \DIFadd{(note that we now plot infidelity rather than fidelity) }\DIFaddend for the \acrref{FO} \acrref{LCD}, \acrref{BPO} and \acrref{COLD} for $N=5$ spins, \DIFdelbegin \DIFdel{noting }\DIFdelend \DIFaddbegin \DIFadd{observing }\DIFaddend that unlike in the two spin case, the \acrref{LCD} approach for this set of operators no longer shows a significant speed up, with final state fidelities remaining low even at long times. In fact, for all plotted times, it shows barely a $1\%$ improvement in fidelity over the bare Hamiltonian, which is not plotted as it overlaps with the \acrref{LCD} line. The \acrref{BPO} approach, on the other hand, appears to perform better at longer times with a sharp increase in fidelity around what is likely a natural timescale for the system, where it dips below the \DIFaddbegin \DIFadd{results produced by }\DIFaddend \acrref{COLD}. \DIFaddbegin \DIFadd{This is likely the phenomenon of a `magic time', as identified in \mbox{%DIFAUXCMD
\cite{couvert_optimal_2008, kiely_fast_2021}}\hskip0pt%DIFAUXCMD
. }\DIFaddend The \acrref{COLD} approach, as before, performs really well at short driving times, where it is orders of magnitude better than either \acrref{LCD} or \acrref{BPO}. At longer times, this advantage wanes whether due to the optimisation process as multiple minima emerge in the cost function landscape or due to the fact that the local non-adiabatic effects are no longer the main source of losses. 
...
While it is all well and good to talk about practical protocols implementing only local \acrref{LCD} operators with control drives that can be accessed by real experiments, one thing that we have so far failed to mention and which is not hard to observe from the form of the \acrref{CD} drive in Eq.~\DIFdelbegin \DIFdel{\ref{eq:CD_Hamiltonian}}\DIFdelend \DIFaddbegin \DIFadd{\eqref{eq:CD_Hamiltonian}}\DIFaddend , is that the amount of power required for the counterdiabatic pulse at short times scales with the speed of the changing Hamiltonian due to the $\dotlambda$ coefficient \DIFaddbegin \DIFadd{present in the counterdiabatic term}\DIFaddend . This means that while both \acrref{LCD} and \acrref{COLD} may lead to really high final state fidelities at very short driving times, they might also do this at the cost of impossibly high power requirements for the drives that implement the counterdiabatic component in either approach. \DIFaddbegin \DIFadd{An extensive discussion of this phenomenon and its consequences can be found in \mbox{%DIFAUXCMD
\cite{campbell_trade-off_2017, funo_universal_2017, santos_superadiabatic_2015}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 

We find, (details in Fig.~\ref{fig:ising_maxamp} in Appendix~\ref{app:ising}), that this is indeed what happens in the Ising chain case: as the total time of the evolution is reduced, the maximum amplitude reached by the \acrref{LCD} pulse increases, leading to two orders of magnitude in difference between the power requirements at $\tau = 10^{-3}J_0^{-1}$ and $\tau = 10^{-1}J_0^{-1}$. As one of the goals of \acrref{COLD} as a method is to be practically implementable, this result is inherently counterproductive\DIFaddbegin \DIFadd{, although it may still provide some insights into the diabatic effects experienced by a system}\DIFaddend .
...
It may be that \acrref{COLD} achieves this advantage by pumping power into the tunnelling term, as discussed in the Ising chain example, but we can see in Fig.~\ref{fig:Synthetic}(c) that the maximum amplitude of the tunnelling term tracks that of \acrref{LCD}. A key issue for experiments is the maximum amplitude achievable by a driving term and with this result we can stipulate that \acrref{COLD} is likely to be feasible in the same regimes as \acrref{LCD} in this synthetic lattice system, but with far higher resulting fidelities. There is single outlier at intermediate times as indicated by the single point peaking in maximum amplitude in Fig.~\ref{fig:Synthetic}(c), this is the exception to the rule, where the optimisation has found a marginally higher fidelity (see the corresponding point in Fig.~\ref{fig:Synthetic}(a)) by pumping in more power. \DIFaddbegin \DIFadd{The maximal amplitude plot may also explain the discontinuous jump in infidelities for the }\acrref{BPO} \DIFadd{case that happens at very short times: there is also a discontinuity in the maximal amplitude reached by the drive for the same points. This can hint either at some controllability limit, wherein a minimal amount of time is required for the }\acrref{BPO} \DIFadd{Hamiltonian to shift the population, or more simply this is a consequence of a local optimiser not being able to find a global minimum which may lead to a higher fidelity and a higher driving amplitude. As the total driving time increases, the cost function landscape changes and likely reveals more optimal minima to the optimiser. 
}\DIFaddend 

Furthermore, we explore the \DIFdelbegin \DIFdel{fidelities }\DIFdelend \DIFaddbegin \DIFadd{infidelities }\DIFaddend with increasing system size for both \acrref{BPO} and \acrref{COLD} in Fig.~\ref{fig:Synthetic}(b). While both protocols show a decreasing fidelity with increasing system size as expected, \acrref{COLD} does not suffer from getting stuck in the initial state, which is what happens in the \acrref{BPO} case as \DIFdelbegin \DIFdel{fidelities }\DIFdelend \DIFaddbegin \DIFadd{infidelities }\DIFaddend go to unity for larger systems in Fig.~\ref{fig:Synthetic}(b). This is the same mechanism as for the short driving times in Fig.~\ref{fig:Synthetic}(a). We also find, as plotted in Fig.~\ref{fig:Synthetic}(d), that increasing the number of optimisable parameters $N_k$ in the control pulse does not contribute to an improvement in the results either for \acrref{BPO} or \acrref{COLD}. This means that, at least in this very simple control setting, there is no reason to expect that \acrref{BPO} will outperform \acrref{COLD} by simply adding more complexity to the control pulse.
...
In Ch.~\ref{chap:5_cd_as_costfunc} we discussed the idea of using the \acrref{AGP} operator and its approximations in order to construct cost functions for the optimisation of Hamiltonian paths in parameter space. There are several reasons why one might expect this to be a good idea: for example, a path in the Hamiltonian parameter space which minimises the \acrref{AGP} norm should, in principle, also minimise the non-adiabatic effects experienced by the system when it is driven along that path. Furthermore, such cost functions should be very efficient to compute once they are written in the correct functional form, giving them an advantage over the fidelity cost function that we used in most examples of Ch.\ref{chap:6_Applications_fidelity}\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. Computing the final state fidelity is an approach }\DIFaddend which suffers from increasing complexity and inefficiency with growing system sizes due to requiring access to the system dynamics along the entire path of the evolution.

In this chapter we will motivate the idea of \acrref{AGP}-based cost functions with numerical results, investigating two types of cost functions presented in Ch.~\ref{chap:5_cd_as_costfunc}: minimisation of absolute integrals of the \acrref{LCD} coefficients and minimisation of their maximal amplitudes. We will do this for three different example systems which we covered in the previous chapter: two-spin annealing in\DIFdelbegin \DIFdel{Sec.~\ref{sec:7.1_two_spin_ho}}\DIFdelend , the Ising spin chain\DIFdelbegin \DIFdel{in Sec.~\ref{sec:7.2_ising_ho_lcd} }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend and finally the preparation of maximally entangled GHZ states in systems of frustrated spins\DIFdelbegin \DIFdel{in Sec. ~\ref{sec:7.3_ghz_ho}. In the last example , we will find }\DIFdelend \DIFaddbegin \DIFadd{. The last example will demonstrate }\DIFaddend a situation where this new approach might not be optimal and \DIFaddbegin \DIFadd{we will }\DIFaddend discuss the reasons behind \DIFdelbegin \DIFdel{why}\DIFdelend \DIFaddbegin \DIFadd{this}\DIFaddend , with the goal of understanding \DIFdelbegin \DIFdel{when such an approach is or isn't effective}\DIFdelend \DIFaddbegin \DIFadd{regimes in which one might want to implement the new method}\DIFaddend . 

\section{Return to two-spin annealing}\label{sec:7.1_two_spin_ho}

In Sec.~\ref{sec:5.1_2spin_annealing} we investigated the \acrref{COLD} protocol in the case of a two-spin annealing protocol described by the Hamiltonian \DIFdelbegin \DIFdel{given by }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend Eq.~\eqref{eq:two_spin_hamiltonian}, where the system starts close to the state $\ket{\uparrow\uparrow}$ and is driven towards a superposition of all the symmetric states. We will return to this simple example in order to illustrate how the integral and maximum amplitude cost functions from Sec.~\ref{chap:5_cd_as_costfunc}, given by Eq.~\eqref{eq:COLD_costfunc_integral} and Eq.~\eqref{eq:COLD_costfunc_maximum} respectively, behave when used to optimise the Hamiltonian path in parameter space for \acrref{COLD}.
...
We know that applying the exact \acrref{CD} pulse made up of all of the \acrref{FO} and \acrref{SO} \acrref{LCD} terms returns unit fidelity regardless of driving time or any optimisable parameters, as shown in Fig.~\ref{fig:twospin_fidelities}. We also found, plotted in the same Figure, that applying \acrref{FO} \acrref{LCD} to the problem without any control pulse performed worse than applying \acrref{COLD} with \acrref{FO} terms, where a control pulse is included and the control parameters are optimised for final state fidelity. \DIFdelbegin \DIFdel{In this case , }\DIFdelend \DIFaddbegin \DIFadd{Here we will also aim to optimise the control pulse as before, but in this case }\DIFaddend we will use a series of cost functions constructed in a similar manner to Eq.~\eqref{eq:COLD_costfunc_integral} and Eq.~\eqref{eq:COLD_costfunc_maximum}\DIFdelbegin \DIFdel{in order to optimise the }%DIFDELCMD < \acrref{COLD} %%%
\DIFdel{control pulse without computing the final state fidelity until after the optimisation is done, in order to check how well the optimisation has performed}\DIFdelend . First, we define the cost functions which use the magnitudes of the coefficient integrals as:
...
\begin{equation}\label{eq:twospin_costfunc_amp}
    ...
    \end{aligned}
\end{equation}
which are used to optimise the parameters $\betabb$ by minimising the maximum amplitude of the given \acrref{LCD} coefficients. \DIFaddbegin \DIFadd{We recall that the minimisation of these quantities is not necessarily directly related to the maximisation of final state fidelity, as in each case the system still needs to take some path in finite time from an initial to a final state while experiencing some amount of non-adiabatic losses, unless exact }\acrref{CD} \DIFadd{is implemented. What we expect to change, however, other than the path in parameter space due to varying $\betabb$, is the }\emph{\DIFadd{structure}} \DIFadd{of the non-adiabatic effects, as mandated by the constraints imposed by each of the cost functions. For example, minimising the total power of two-body non-adiabatic terms as in $C_{\rm I, (\gamma + \zeta)}$, might inadvertently maximise the effects of the local non-adiabatic terms $\alpha$ or even more non-local operators. This is because the cost function captures nothing about the behaviour of such terms.
}\DIFaddend 
...
In this thesis we introduced a new method for speeding up adiabatic quantum protocols while minimising losses due to transitions out of the instantaneous eigenstate: \acrref{COLD}. The new method is comprised of two key components: approximate \DIFdelbegin %DIFDELCMD < \acrref{CD} %%%
\DIFdelend \DIFaddbegin \DIFadd{counterdiabatic driving }\DIFaddend techniques and quantum optimal control. We discussed the theoretical framework and motivation behind \acrref{COLD}, beginning in Ch.~\ref{chap:2_adiabaticity} with a background introduction to quantum adiabaticity and the losses which arise as a consequence of fast changes in a time-dependent Hamiltonian. \DIFdelbegin \DIFdel{In Sec.~\ref{sec:2.2_AGP} we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend covered how these losses can be described by an operator known as the \acrref{AGP} \cite{kolodrubetz_geometry_2017}, and \DIFdelbegin \DIFdel{in Sec.~\ref{sec:2.3_CD} we discussed }\DIFdelend how the \acrref{CD} technique can be used to suppress the non-adiabatic effects generated by the \acrref{AGP} \cite{berry_transitionless_2009, demirplak_adiabatic_2003}. We explored the reasons why the exact \acrref{CD} pulse is often inaccessible, either in theory or in practice, \DIFdelbegin \DIFdel{in Sec.~\ref{sec:2.4_approximate_CD}, }\DIFdelend and introduced several existing methods for constructing an \emph{approximate} counterdiabatic drive. Then, in Ch.~\ref{chap:3_Quantum_Optimal_control}, we covered the theory and methodology involved in optimal control theory, which concerns itself with finding optimal path for a system from some initial state to some final state. \DIFdelbegin \DIFdel{In Sec.~\ref{sec:3.2_Quantum_optimal_control} we introduced }\DIFdelend \DIFaddbegin \DIFadd{We introduced ideas concerning }\DIFaddend how optimal control theory can be applied in the setting of quantum systems and \DIFdelbegin \DIFdel{in Sec.~\ref{sec:3.3_qoct_methods} we introduced }\DIFdelend \DIFaddbegin \DIFadd{then we described }\DIFaddend several popular quantum optimal control methods like \acrref{CRAB} and \acrref{GRAPE}. 

These two background chapters paved the way for \DIFdelbegin \DIFdel{a detailed introduction to }\DIFdelend \acrref{COLD} in Part~\ref{part:COLD}. \DIFdelbegin \DIFdel{The new method , }%DIFDELCMD < \acrref{COLD}%%%
\DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{This new method }\DIFaddend is the result of combining an approximate \acrref{CD} method we refer to as \acrref{LCD} \cite{sels_minimizing_2017}, with quantum optimal control techniques. \DIFdelbegin \DIFdel{In Sec.~\ref{sec:2.4.1_LCD}, we outlined , }\DIFdelend \DIFaddbegin \DIFadd{Having outlined }\DIFaddend how \acrref{LCD} \DIFdelbegin \DIFdel{works by allowing }\DIFdelend \DIFaddbegin \DIFadd{allows }\DIFaddend one to variationally determine a pulse shape for an ansatz set of physical operators which most closely resembles the exact counterdiabatic drive for a given time-dependent Hamiltonian. The goal of \acrref{LCD} is to suppress as many losses associated with such transitions as possible within the restrictions imposed by the ansatz set of operators. In Ch.~\ref{chap:4_COLD}, we described how \acrref{COLD} can improve upon the \acrref{LCD} approach by using the observation that the non-adiabatic effects experienced by a system driven by a time-dependent Hamiltonian depend on the path of the Hamiltonian through parameter space. We showed how this path can be changed via the implementation of methods from optimal control theory\DIFdelbegin \DIFdel{in Sec.~\ref{sec:4.2_COLD_QOCT}}\DIFdelend , thus allowing \acrref{COLD} to find a path which maximises the effects of the \acrref{LCD} for a given ansatz set of operators. We then posited, in Ch.~\ref{chap:5_cd_as_costfunc}, that the information about non-adiabatic effects contained in the \acrref{AGP} operator could be used to construct optimisation metrics for the optimal control component of \acrref{COLD}.
...
\DIFdelbegin \DIFdel{A particular exampleof ideas inspired by }%DIFDELCMD < \acrref{COLD}%%%
...
Beyond these ideas\DIFdelbegin \DIFdel{and considerations}\DIFdelend , there may be better approaches to computing the approximate \acrref{CD} components, which is occasionally an arduous task (see \@e.g.~Appendix~\ref{app:arbitrary_ising_derivation}). This is the goal of \DIFdelbegin \DIFdel{some upcoming work \mbox{%DIFAUXCMD
\cite{lawrence_numerical_2023}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{lawrence_numerical_2024}}\hskip0pt%DIFAUXCMD
}\DIFaddend , which explores how the structure in certain Hamiltonians like the Ising model can be exploited in order to compute the \acrref{LCD} coefficients for a large numbers of operators very efficiently. Should this be accomplished in a more general setting, \acrref{AGP}-based cost functions may become an even greater resource, as we might be able to better characterise the behaviour of different orders of \acrref{LCD} with respect to each other and the target state. The exact \acrref{AGP} operator \DIFdelbegin \DIFdel{, which not only forms the backbone of }%DIFDELCMD < \acrref{CD} %%%
\DIFdel{\mbox{%DIFAUXCMD
\cite{kolodrubetz_geometry_2017, sels_minimizing_2017}}\hskip0pt%DIFAUXCMD
, but also acts as a probe for quantum chaos \mbox{%DIFAUXCMD
\cite{pandey_adiabatic_2020, bhattacharjee_lanczos_2023}}\hskip0pt%DIFAUXCMD
...
G.~Pelegrí, A.~J. Daley, and J.~D. Pritchard,
  ...
  Publishing.}\DIFdelend \DIFaddbegin \DIFadd{2022. }\DIFaddend [Online]\DIFaddbegin \DIFadd{.
  Available: }\url{https://dx.doi.org/10.1088/2058-9565/ac823a}
\BIBentrySTDinterwordspacing

\bibitem{albash_adiabatic_2018}
\BIBentryALTinterwordspacing
\DIFadd{T.~Albash and D.~A. Lidar, ``Adiabatic quantum computation,'' }\emph{\DIFadd{Reviews of
  Modern Physics}}\DIFadd{, vol.~90, no.~1,  015002, Jan. 2018. }[\DIFadd{Online}]\DIFaddend . Available:
  \DIFdelbegin %DIFDELCMD < \url{https://dx.doi.org/10.1088/2058-9565/ac823a}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \url{https://link.aps.org/doi/10.1103/RevModPhys.90.015002}
...
\DIFdelend \DIFaddbegin \bibitem{sugiura_adiabatic_2021}
\DIFaddend \BIBentryALTinterwordspacing
S.~Sugiura, P.~W. Claeys, A.~Dymarsky, and A.~Polkovnikov, ``Adiabatic
  landscape and optimal paths in ergodic systems,'' \emph{\DIFdelbegin \DIFdel{arXiv:2004.13735
  }%DIFDELCMD < [%%%
\DIFdel{cond-mat, physics:hep-th, physics:quant-ph}%DIFDELCMD < ]%%%
\DIFdelend \DIFaddbegin \DIFadd{Physical Review
  Research}\DIFaddend }, \DIFdelbegin \DIFdel{Apr.2020, arXiv: 2004.13735}\DIFdelend \DIFaddbegin \DIFadd{vol.~3, no.~1,  013102, Feb. 2021, publisher: American Physical
  Society}\DIFaddend . [Online]. Available:
  \DIFdelbegin %DIFDELCMD < \url{http://arxiv.org/abs/2004.13735}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \url{https://link.aps.org/doi/10.1103/PhysRevResearch.3.013102}
\DIFaddend \BIBentrySTDinterwordspacing

\bibitem{sels_minimizing_2017}
\BIBentryALTinterwordspacing
...
\bibitem{wiseman_quantum_2009}
\BIBentryALTinterwordspacing
\DIFadd{H.~M. Wiseman and G.~J. Milburn, }\DIFadd{Quantum }\DIFadd{Measurement} \DIFadd{and
  }\DIFadd{Control}.\DIFadd{Cambridge: Cambridge University Press, 2009. }\DIFadd{[Online]}\DIFadd{. }\DIFaddend Available:
  \DIFdelbegin %DIFDELCMD < \url{https://link.aps.org/doi/10.1103/PhysRevResearch.4.L012029}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \url{https://www.cambridge.org/core/books/quantum-measurement-and-control/F78F445CD9AF00B10593405E9BAC6B9F}
\DIFaddend \BIBentrySTDinterwordspacing
...
J.~Nocedal and S.~Wright, \emph{\BIBforeignlanguage{en}{Numerical
  ...
  Business Media, Dec. \DIFdelbegin \DIFdel{2006, google-Books-ID: VbHYoSyelFcC.}\DIFdelend \DIFaddbegin \DIFadd{2006.
}\DIFaddend 

\DIFaddbegin \bibitem{kingma_adam_2015}
\BIBentryALTinterwordspacing
\DIFadd{D.~P. Kingma and J.~Ba, ``Adam: }{\DIFadd{A}} {\DIFadd{Method}} \DIFadd{for }{\DIFadd{Stochastic}} {\DIFadd{Optimization}}\DIFadd{,''
  in }\emph{\DIFadd{3rd }{\DIFadd{International}} {\DIFadd{Conference}} \DIFadd{on }{\DIFadd{Learning}} {\DIFadd{Representations}}\DIFadd{,
  }{\DIFadd{ICLR}} \DIFadd{2015, }{\DIFadd{San}} {\DIFadd{Diego}}\DIFadd{, }{\DIFadd{CA}}\DIFadd{, }{\DIFadd{USA}}\DIFadd{, }{\DIFadd{May}} \DIFadd{7-9, 2015, }{\DIFadd{Conference}}
  {\DIFadd{Track}} {\DIFadd{Proceedings}}}\DIFadd{, Y.~Bengio and Y.~LeCun, Eds., 2015. }[\DIFadd{Online}]\DIFadd{.
  Available: }\url{http://arxiv.org/abs/1412.6980}
\BIBentrySTDinterwordspacing

\DIFaddend \bibitem{suli_introduction_2003}
\BIBentryALTinterwordspacing
...
\DIFaddbegin \bibitem{hestenes_methods_1952}
\BIBentryALTinterwordspacing
\DIFadd{M.~Hestenes and E.~Stiefel, ``}\BIBforeignlanguage{en}{Methods of conjugate
  gradients for solving linear systems}\DIFadd{,''
  }\emph{\BIBforeignlanguage{en}{Journal of Research of the National Bureau of
  Standards}}\DIFadd{, vol.~49, no.~6,  409, Dec. 1952. }[\DIFadd{Online}]\DIFadd{. Available:
  }\url{https://nvlpubs.nist.gov/nistpubs/jres/049/jresv49n6p409_A1b.pdf}
\BIBentrySTDinterwordspacing

\DIFaddend \bibitem{brent_algorithms_2002}
...
\DIFaddbegin \bibitem{ban_counter-diabatic_2014}
\BIBentryALTinterwordspacing
\DIFadd{Y.~Ban and X.~Chen, ``}\BIBforeignlanguage{en}{Counter-diabatic driving for fast
  spin control in a two-electron double quantum dot}\DIFadd{,''
  }\emph{\BIBforeignlanguage{en}{Scientific Reports}}\DIFadd{, vol.~4, no.~1,  6258,
  Sep. 2014, publisher: Nature Publishing Group. }[\DIFadd{Online}]\DIFadd{. Available:
  }\url{https://www.nature.com/articles/srep06258}
\BIBentrySTDinterwordspacing

\DIFaddend \bibitem{saberi_adiabatic_2014}
\BIBentryALTinterwordspacing
...
\DIFdelbegin \bibitem{albash_adiabatic_2018}
\DIFdelend \DIFaddbegin \bibitem{petersen_control_1987}
\DIFaddend \BIBentryALTinterwordspacing
\DIFdelbegin \DIFdel{T.~Albash and D.~A. Lidar, ``Adiabatic quantum computation,''
  }\DIFdelend \DIFaddbegin \DIFadd{I.~R. Petersen and B.~Ross~Barmish, ``}\BIBforeignlanguage{en}{Control effort
  considerations in the stabilization of uncertain dynamical systems}\DIFadd{,''
  }\DIFaddend \emph{\DIFdelbegin \DIFdel{Reviews of
  Modern Physics}\DIFdelend \DIFaddbegin \BIBforeignlanguage{en}{Systems \& Control Letters}\DIFaddend }, vol.~\DIFdelbegin \DIFdel{90}\DIFdelend \DIFaddbegin \DIFadd{9}\DIFaddend , no.~\DIFdelbegin \DIFdel{1,
  p. 015002, Jan. 2018, publisher: American
  Physical Society. }\DIFdelend \DIFaddbegin \DIFadd{5,
  417--422, Nov. 1987. }\DIFaddend [Online]. Available:
  \DIFdelbegin %DIFDELCMD < \url{https://link.aps.org/doi/10.1103/RevModPhys.90.015002}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \url{https://www.sciencedirect.com/science/article/pii/0167691187900715}
\DIFaddend \BIBentrySTDinterwordspacing

\DIFdelbegin \bibitem{petersen_control_1987}
\DIFdelend \DIFaddbegin \bibitem{abah_energetic_2019}
\DIFaddend \BIBentryALTinterwordspacing
\DIFdelbegin \DIFdel{I.~R. Petersen and B.~Ross~Barmish, ``}%DIFDELCMD < \BIBforeignlanguage{en}{Control effort
%DIFDELCMD <   considerations in the stabilization of uncertain dynamical systems}%%%
\DIFdel{,''
  }\DIFdelend \DIFaddbegin \DIFadd{O.~Abah, R.~Puebla, A.~Kiely, G.~De~Chiara, M.~Paternostro, and S.~Campbell,
  ``}\BIBforeignlanguage{en}{Energetic cost of quantum control protocols}\DIFadd{,''
  }\DIFaddend \emph{\DIFdelbegin %DIFDELCMD < \BIBforeignlanguage{en}{Systems \& Control Letters}%%%
\DIFdelend \DIFaddbegin \BIBforeignlanguage{en}{New Journal of Physics}\DIFaddend }, vol.~\DIFdelbegin \DIFdel{9}\DIFdelend \DIFaddbegin \DIFadd{21}\DIFaddend , no.~\DIFdelbegin \DIFdel{5,
  pp. 417--422, Nov. 1987. }\DIFdelend \DIFaddbegin \DIFadd{10,
  1367--2630, Oct. 2019. }\DIFaddend [Online]. Available:
  \DIFdelbegin %DIFDELCMD < \url{https://www.sciencedirect.com/science/article/pii/0167691187900715}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \url{https://iopscience.iop.org/article/10.1088/1367-2630/ab4c8c}
\DIFaddend \BIBentrySTDinterwordspacing
...
\DIFdelbegin \bibitem{lang_topological_2017}
\DIFdelend \DIFaddbegin \bibitem{couvert_optimal_2008}
\DIFaddend \BIBentryALTinterwordspacing
\DIFdelbegin \DIFdel{N.~Lang and H.~P.Büchler, ``}%DIFDELCMD < \BIBforeignlanguage{en}{Topological networks for
%DIFDELCMD <   quantum communication between distant qubits}%%%
\DIFdel{,'' }\DIFdelend \DIFaddbegin \DIFadd{A.~Couvert, T.~Kawalec, G.~Reinaudi, and D.~Guéry-Odelin,
  ``}\BIBforeignlanguage{en}{Optimal transport of ultracold atoms in the
  non-adiabatic regime}\DIFadd{,'' }\emph{\BIBforeignlanguage{en}{Europhysics Letters}}\DIFadd{,
  vol.~83, no.~1,  13001, Jun. 2008. }[\DIFadd{Online}]\DIFadd{. Available:
  }\url{https://dx.doi.org/10.1209/0295-5075/83/13001}
\BIBentrySTDinterwordspacing

\bibitem{kiely_fast_2021}
\BIBentryALTinterwordspacing
\DIFadd{A.~Kiely and S.~Campbell, ``}\BIBforeignlanguage{en}{Fast and robust magnon
  transport in a spin chain}\DIFadd{,'' }\DIFaddend \emph{\DIFdelbegin %DIFDELCMD < \BIBforeignlanguage{en}{npj Quantum Information}%%%
\DIFdelend \DIFaddbegin \BIBforeignlanguage{en}{New Journal of
  Physics}\DIFaddend }, vol.~\DIFaddbegin \DIFadd{23, no.~}\DIFaddend 3,  \DIFaddbegin \DIFadd{033033, Mar. 2021, publisher: IOP Publishing.
  }[\DIFadd{Online}]\DIFadd{. Available: }\url{https://dx.doi.org/10.1088/1367-2630/abea43}
\BIBentrySTDinterwordspacing

\bibitem{campbell_trade-off_2017}
\BIBentryALTinterwordspacing
\DIFadd{S.~Campbell and S.~Deffner, ``Trade-}{\DIFadd{Off}} {\DIFadd{Between}} {\DIFadd{Speed}} \DIFadd{and }{\DIFadd{Cost}} \DIFadd{in
  }{\DIFadd{Shortcuts}} \DIFadd{to }{\DIFadd{Adiabaticity}}\DIFadd{,'' }\emph{\DIFadd{Physical Review Letters}}\DIFadd{, vol. 118,
  }\DIFaddend no.~\DIFdelbegin \DIFdel{1,  pp. 1--10, Nov. }\DIFdelend \DIFaddbegin \DIFadd{10,  100601, Mar. 2017, publisher: American Physical Society. }[\DIFadd{Online}]\DIFadd{.
  Available: }\url{https://link.aps.org/doi/10.1103/PhysRevLett.118.100601}
\BIBentrySTDinterwordspacing

\bibitem{funo_universal_2017}
\BIBentryALTinterwordspacing
\DIFadd{K.~Funo, J.-N. Zhang, C.~Chatou, K.~Kim, M.~Ueda, and A.~del Campo, ``Universal
  }{\DIFadd{Work}} {\DIFadd{Fluctuations}} {\DIFadd{During}} {\DIFadd{Shortcuts}} \DIFadd{to }{\DIFadd{Adiabaticity}} \DIFadd{by
  }{\DIFadd{Counterdiabatic}} {\DIFadd{Driving}}\DIFadd{,'' }\emph{\DIFadd{Physical Review Letters}}\DIFadd{, vol. 118,
  no.~10,  100602, Mar. }\DIFaddend 2017, \DIFdelbegin \DIFdel{number: }\DIFdelend \DIFaddbegin \DIFadd{publisher: American Physical Society. }[\DIFadd{Online}]\DIFadd{.
  Available: }\url{https://link.aps.org/doi/10.1103/PhysRevLett.118.100602}
\BIBentrySTDinterwordspacing

\bibitem{santos_superadiabatic_2015}
\BIBentryALTinterwordspacing
\DIFadd{A.~C. Santos and M.~S. Sarandy, ``}\BIBforeignlanguage{en}{Superadiabatic
  {Controlled} {Evolutions} and {Universal} {Quantum} {Computation}}\DIFadd{,''
  }\emph{\BIBforeignlanguage{en}{Scientific Reports}}\DIFadd{, vol.~5, no.~}\DIFaddend 1\DIFdelbegin \DIFdel{Publisher}\DIFdelend \DIFaddbegin \DIFadd{,  15775,
  Oct. 2015, publisher}\DIFaddend : Nature Publishing Group. [Online]. Available:
  \DIFaddbegin \url{https://www.nature.com/articles/srep15775}
\BIBentrySTDinterwordspacing

\bibitem{lang_topological_2017}
\BIBentryALTinterwordspacing
\DIFadd{N.~Lang and H.~P. Büchler, ``}\BIBforeignlanguage{en}{Topological networks for
  quantum communication between distant qubits}\DIFadd{,''
  }\emph{\BIBforeignlanguage{en}{npj Quantum Information}}\DIFadd{, vol.~3, no.~1, ~47,
  Nov. 2017. }[\DIFadd{Online}]\DIFadd{. Available:
  }\DIFaddend \url{https://www.nature.com/articles/s41534-017-0047-x}
\BIBentrySTDinterwordspacing
...
\DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{Ph.D. dissertation, Sep. 2011. }[\DIFadd{Online}]\DIFadd{. Available:
  }\url{https://dx.doi.org/10.1088/0953-4075/44/19/193001}
\BIBentrySTDinterwordspacing

\bibitem{coopmans_optimal_2022}
\BIBentryALTinterwordspacing
\DIFadd{L.~Coopmans, S.~Campbell, G.~De~Chiara, and A.~Kiely, ``Optimal control in
  disordered quantum systems,'' }\emph{\DIFadd{Physical Review Research}}\DIFadd{, }\DIFaddend vol.~\DIFdelbegin \DIFdel{44}\DIFdelend \DIFaddbegin \DIFadd{4}\DIFaddend , no.~\DIFdelbegin \DIFdel{19,
   p. 193001, Sep. 2011. }\DIFdelend \DIFaddbegin \DIFadd{4,
   043138, Nov. 2022, publisher: American Physical Society. }\DIFaddend [Online].
  Available: \DIFdelbegin %DIFDELCMD < \url{https://dx.doi.org/10.1088/0953-4075/44/19/193001}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \url{https://link.aps.org/doi/10.1103/PhysRevResearch.4.043138}
\DIFaddend \BIBentrySTDinterwordspacing
